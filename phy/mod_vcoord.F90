...
contains
   ...
   subroutine cntiso_regrid_nudge_jslice(p_src, p_dst, stab_fac, &
                                         i_lb, i_ub, j, j_rs, nn)
      ...
      real(r8) :: dpmin_inflation_factor_i, sig_max, dpmin_sfc, &
                  ...
                  sig_intrp, q
      integer :: l, i, nt, k, kt, kl, ktzmin, ktzmax, errstat
      ...
      do i = max(i_lb, ifp(j, l)), min(i_ub, ilp(j, l))
         ...
         ! density of this transition interface. A layer range above and below
         ! the transition interface may be specified, making a transition zone
         ! where interface reference potential densities are adjusted to achieve
         ! a more gradual change from pressure level to isopycnic interfaces.
         sig_trg(:) = sigmar_1d(:)
         ...
         do while (kt <= kdmx(i))
            ...
            if (sigmar_1d(kt) > sig_pmin(kt)) then
               ...
               if (ktzmin < kt .and. ktzmax - ktzmin > 1) then
                  ...
                  ! boundary of the transition zone. If construction of a Bezier
                  ! curve fails, use a linear change of reference potential
                  ! densities in the transition zone.
                  ...
                  if (dsigdx_lo <= dsigdx_up .or. &
                      sig_up - sig_lo + dsigdx_lo <= 0 .or. &
                      sig_up - sig_lo + dsigdx_lo >= dsigdx_lo - dsigdx_up) then
                     ...
                     xi = (sig_up - sig_lo + dsigdx_lo) &
                          /(dsigdx_lo - dsigdx_up)
                     si = (dsigdx_lo*(sig_up + dsigdx_up) - dsigdx_up*sig_lo) &
                          /(dsigdx_lo - dsigdx_up)
            ...
            p_dst(kt,i) = &
               min(max(p_dst(kt,i), pmin(kt), p_dst(kt-1,i) + dpmin_interior), &
                   p_src(kk+1,i))
            stab_fac(kt,i) = 0._r8
            kt = kt + 1
         enddo
   ...
   subroutine cntiso_regrid_smooth_jslice(p_dst_rs, stab_fac_rs, &
      ...
      do i = max(i_lb, ifu(j+1,l)), min(i_ub+1, ilu(j+1,l))
         ...
         do k = 2, kk
            ...
            sdiff = min((stab_fac_limit - q)*smooth_diff_max/stab_fac_limit, &
                        difmx)
...
!#  endif
         ...
         j = 0
         call ndiff_vflx_jslice(p_dst_rs, 1, ii, j+1, jm_rs, jp_rs, mm, nn)
...
!#  endif
         ...
         call prep_recon_jslice(p_src_rs(:,:,jn_rs), &
                                -1, ii+2, j+2, jn_rs, nn)
         call recon_trc_jslice(-1, ii+2, j+2, jn_rs, nn)
